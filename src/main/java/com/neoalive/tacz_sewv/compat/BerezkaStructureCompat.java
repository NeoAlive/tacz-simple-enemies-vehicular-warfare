...
public final class BerezkaStructureCompat {
    ...
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger("tacz_sewv/berezka");
    ...
    public static void onStructureSpawned(onStructureSpawned event) {
        ...
        if (level == null) {
            LOG.info("structure {} at {}: no current world, skipped", event.getStructureName(), event.getBlockPos());
            return;
        }

        TankSpawner.TankFaction faction = factionFor(event.getStructureName());
        if (faction == null) return;
        LOG.info("structure {} at {} -> {} vehicles", event.getStructureName(), event.getBlockPos(), faction);
        ...
        // MineColoniesCompat.
        if (!com.neoalive.tacz_sewv.spawn.AmbientSpawnGate.allowsAt(level, anchor)) {
            LOG.info("  blocked by ambient spawn gate");
            return;
        }
        ...
        // berezka posts this from a worldgen worker, while the structure's chunk is still being
        // generated — it is NOT loaded yet when the main thread first sees it, so a one-shot check
        // always failed. Queue it and let the server tick spawn once the chunk is actually full,
        // never force-generating neighbours from the frontier (that cascades into more events).
        PENDING.add(new Pending(level, anchor, bounds, faction, level.getGameTime() + PENDING_TTL_TICKS));
    }

    private record Pending(ServerLevel level, BlockPos anchor, @Nullable BoundingBox bounds,
                           TankSpawner.TankFaction faction, long expiresAt) {}

    /** Two minutes: long enough for a chunk that is genuinely on its way in, short of a stale leak. */
    private static final long PENDING_TTL_TICKS = 2400;
    ...
    public static void onServerTick(TickEvent.ServerTickEvent event) {
        ...
        for (int n = PENDING.size(); n > 0; n--) {
            Pending p = PENDING.poll();
            if (p == null) return;
            if (p.level().isLoaded(p.anchor())) {
                spawnVehicles(p.level(), p.anchor(), p.bounds(), p.faction());
            } else if (p.level().getGameTime() < p.expiresAt()) {
                PENDING.add(p);
            } else {
                LOG.info("  anchor {} never loaded, gave up", p.anchor());
            }
        }
    }
    ...
    private static void spawnVehicles(ServerLevel level, BlockPos anchor, @Nullable BoundingBox bounds,
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!level.isLoaded(pos)) {
                LOG.info("  hull {} at {} not loaded, skipped", i, pos);
                continue;
            }
            ...
            else LOG.info("  hull {} at {}: spawnTankWithCrew returned null", i, pos);
        }
    }
