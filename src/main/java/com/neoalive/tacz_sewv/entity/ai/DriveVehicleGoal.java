...
public class DriveVehicleGoal extends Goal {
    ...
    public void tick() {
        ...
        if (distanceSq > stopDistance * stopDistance) {
            ...
            stopVehicleMovement(); // close enough, halt
        }
    }
    ...
    private void driveGroundVehicle(BlockPos targetPos, double distanceSq) {
        Vec3 toTarget = new Vec3(
                ...
                0, // ignore vertical for ground steering
        ...
        if (Math.abs(angle) < angleThreshold) {
            // Facing the target — drive forward
            moveVehicleForward();
        } else {
            // Need to turn — steer toward target
            this.vehicle.setLeftInputDown(angle > 0);
            this.vehicle.setRightInputDown(angle < 0);
            this.vehicle.setForwardInputDown(true); // MVP: turn while moving (wheeled-style)
