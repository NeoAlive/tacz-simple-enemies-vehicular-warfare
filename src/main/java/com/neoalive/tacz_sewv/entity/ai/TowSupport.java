...
public final class TowSupport {
     ...
     *       reload, not the shot.
     *   <li>{@code countBackupAmmo} — the crew has a round to give. Deliberately not a
     *       hardcoded {@code MEDIUM_ANTI_GROUND_MISSILE} test: this asks the weapon's own
     *       AmmoConsumer, so it follows the datapack if a pack repoints the TOW's AmmoType,
     *       and it picks up a creative ammo box (which answers {@code Integer.MAX_VALUE}) —
     *       the way to give a crew unlimited missiles for testing.
     * </ul>
     *
     * <p>The load itself goes through {@code GunData.reloadAmmo}, which resolves the crew's
     * inventory through the {@code ITEM_HANDLER} capability rather than anything
     * player-specific, so an SEM unit is a perfectly good ammo source. {@code reloadAmmo(Entity)}
     * is byte-for-byte the call {@code TowEntity.interact} makes for a player.
     *
    ...
    public static boolean reload(TowEntity tow, AbstractUnit unit) {
        try {
            ...
            if (gun.countBackupAmmo(unit) <= 0) return false;

            tow.modifyGunData(GUNNER_SEAT, data -> data.reloadAmmo(unit));
            tow.setLoaded(true);
